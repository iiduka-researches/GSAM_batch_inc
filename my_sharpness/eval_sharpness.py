...
def get_args():
    ...
    parser.add_argument('--model', default='resnet18',choices=['resnet18', "wide_resnet28-10","resnet50","ViT-T","ViT-S"], type=str)
...
model = my_models.get_model(args.model).to(device).eval()
...
model = my_models.LogitNormalizationWrapper(model, normalize_logits=args.normalize_logits)
